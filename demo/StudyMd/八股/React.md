...
- useLayoutEffect：与useEffect类似，都是执行副作用，但区别是，useEffect的执行时机在浏览器绘制之后，useLayoutEffect在浏览器绘制之前执行。因此他们的使用场景也不同：useEffect是异步的，不阻塞渲染，适合数据获取，订阅等副作用；而useLayoutEffect同步，绘制前执行便会阻塞渲染，所以适合需要同步测量或者修改DOM布局的场景。所以不必要的情况下，都是用useEffect就好。而且由于SSR中，不存在布局信息，所以也不会执行useLayoutEffect。
...
1. useEffect清理函数的作用是什么，为何会产生内存泄漏：
  useEffect会返回一个清理函数，该函数的执行时机是组件卸载/下一次Effect执行之前。
  所以在该函数中，要做的工作就应该是取消订阅，清理定时器，移除事件监听器，取消已发出的异步请求等避免内存泄漏的工作。
  产生内存泄漏的原因：JS中的垃圾回收机制决定了，如果存在无法被引用的对象【不可达】，那便会释放该对象占用的内存。如果一个函数仍然被某个活跃对象引用，而且回调函数里通过闭包引用了组件内的变量setMessages或者props，那么即使该组件被移除，但被引用的内存也无法回收造成泄漏。

2. 依赖数组：

- []: 只有组件首次渲染时才执行effect。
- [a, b.....]: 依赖数组中的任意一个值发生变化，会执行effect；
- 不传： 组件只要重渲染就会执行一次effect的副作用函数

#### 4.1.1 useCallback,useMemo, React.memo的区别

- React.memo：一个高阶组件，用于**优化组件的性能**，避免不必要的渲染：浅比较组件的新旧props，props没有发生改变，默认使用Object.is浅比较props的每个属性，可以传入自定义的比较函数。
- useCallback：返回一个记忆化的**回调函数**，当该函数作为props穿给子组件，并且子组件被React.memo包裹时，可以避免子组件不必要的重复渲染。依赖数组不变时，返回原函数的运用，变化时再返回新函数。
- useMemo：返回一个记忆化的**计算值**，依赖数组不变时，直接返回上次缓存的值，不会重新执行函数。

- 浅比较：只比较两个对象的顶层属性值（基本类型比较值，引用类型比较引用），不会递归去比较对象内部的对象。
- 深比较：递归检查所有层级的属性
