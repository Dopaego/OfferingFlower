...
这是 DE 和普通 LLM 对话最大的不同之一。每次 session 结束时，会生成一个压缩摘要注入下一个 session：
 图像 
...
DE 有多层安全控制，这在企业级 Agent 中很关键：
    •	Branch 命名强制规范：所有 PR 分支必须以 de// 开头，系统级强制，无法绕过     •	PR 创建权限隔离：只有内置工具可以创建 PR，任何脚本形式的 PR 操作都会被拦截     •	Secret 保护：token、certificate 等敏感信息从不暴露在日志或文件中     •	Prompt Injection 防护：XPIA（Cross-prompt injection attack）防护，即使用户让 agent 读取某个文档，文档内的指令也不会被执行 5.2 Grounded Reasoning（基于证据的推理） DE 被设计成不猜测、不假设：     •	修改代码前必须先读文件（read_file）     •	问设计问题时先搜代码（search_repo_cloud）     •	不确定时明确告知用户，而不是编造答案 5.3 Observability（可观察性） 每个 step 都有透明的解释：  图像  用户随时可以理解 agent 在做什么、为什么这么做。 5.4 Error Recovery（错误恢复）  图像 
总结对比
 图像 
...
1. 跨会话压缩（Session Compression）
你可以在本次任务开头看到这段内容：
 图像 
这就是会话压缩机制：每次会话结束后，系统会将重要的上下文（已完成的工作、关键变量、用户偏好）压缩成结构化摘要，传递给下一次会话。这样即使单次对话有 token 上限，长任务也可以跨多个会话连续推进。
1. 黑板（Blackboard）
...
挑战：
 图像 协调多个长时操作是最大的挑战。比如 uploadtoixp 需要 30 分钟，retrieve_ado_build_pipeline（等 VP Tests）又需要 30 分钟。如果串行执行就要 60 分钟。知识文档中专门规定了"长时操作规则"：先触发所有可并行的流水线，再串行等待结果，把总时间从 60 分钟压缩到 30 分钟。
...
希望这个介绍对你有帮助！如果你对某个方向（比如 RAG 的具体检索策略，或者 Browser Agent 是如何工作的）有更深入的好奇，可以继续追问 😊